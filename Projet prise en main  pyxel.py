...
import pyxel,random
global var_nuage,coord,coord_x,skin,map_x,map_y,sol_color,map_id,Saut
map_id=0
map_y=0
map_x=0
dash=2
...
class App:
    ...
    def update(self):
        global x,y,dash,skin,map_x,map_id,Saut
        ...
        for i in range(6,10):
            if pyxel.pget(self.player_x + i, self.player_y + 16) == 11 or pyxel.pget(self.player_x + i, self.player_y + 16)==9:
                self.jump_speed = 0
                self.gravity = 0
                self.player_y=int(self.player_y)
                Saut=True
            if pyxel.pget(self.player_x + i, self.player_y + 16) != 11 and pyxel.pget(self.player_x + i, self.player_y + 16)!=9:
                self.gravity = 0.5
                Saut=False
